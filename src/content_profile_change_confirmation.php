...
* @version $Id: content_profile_change_confirmation.php 39600 2014-03-18 11:43:38Z weinert $
*/
    ...
    if (!$surferObj->isValid && $this->data['password_required'] == 1) {
      ...
      if (isset($this->params['mail_token']) && isset($this->params['password'])) {
        ...
        if (!empty($surferId)) {
          ...
          if (
              is_array($surfer) && isset($surfer['surfer_password']) &&
              $surferAdmin->passwordApi()->verifyPassword(
                $this->params['password'], $surfer['surfer_password']
              )
             ) {
