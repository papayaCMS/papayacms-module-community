...
* @version $Id: content_avatarupload.php 39733 2014-04-08 18:10:55Z weinert $
*/
...
class content_avatarupload extends base_content {
  ...
  function getParsedData($parseParams = NULL) {
    ...
    if (isset($surferObj) && is_object($surferObj) && $surferObj->isValid) {
      if ($this->checkSurferPerm($this->data['perm_upload_avatar'])) {
        $this->initializeMediaDBEditObject();
        $surferData = $this->surferAdmin->loadSurfer($surferObj->surferId, TRUE);
        ...
        if (isset($this->params['upload']) && $this->params['upload'] == 1) {
          if (@isset($_FILES[$this->paramName]['tmp_name']['surfer_avatar'])) {
            ...
            if (!empty($mediaId) && PapayaFilterFactory::isGuid($mediaId, TRUE)) {
    ...
    $result .= sprintf(
      '<avatar src="%s" />'.LF,
      $this->surferAdmin->getAvatar($surferObj->surferId, 0, TRUE, 'max')
    );
