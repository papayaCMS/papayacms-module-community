...
* @version $Id: content_showuser_list.php 39732 2014-04-08 15:34:45Z weinert $
*/
      ...
      if ($surferAdmin->editSurfer['surfergroup_id'] == $this->data['user_group']) {
        ...
        if (isset($this->data['show_email']) &&
          ...
          if (isset($_POST['submit'])) {
            ...
            !PapayaFilterFactory::isText($this->params['mail_subject'], TRUE)) {
              $inputError = TRUE;
            }
            if (empty($this->params['mail_text']) ||
            !PapayaFilterFactory::isText($this->params['mail_text'], TRUE)) {
              $inputError = TRUE;
            }
            if ($inputError) {
              $result .= sprintf(
              ...
              $emailObj->setSender($this->surferObj->surfer['surfer_email']);
              if ($emailObj->send($recipient, $subject, $body)) {
                $result .= sprintf(
                  '<message>%s</message>',
