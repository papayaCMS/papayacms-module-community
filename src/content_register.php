...
class content_register extends base_content {
  ...
  */
  ...
  /**
  * Base surfers
  * @var surfer_admin $baseSurfers surfer_admin
  */
    ...
    array(
      ...
      array(
        ...
        'auto_login' => array(
          ...
          'yesno',
          '',
          'Automatically log in after opt-in', 0
        ),
        'auto_login_redirect' => array(
          'Auto login redirect',
          'isNum',
          FALSE,
          'pageid',
          10,
          'Provide this if you want to redirect users after automatic login',
          0
        ),
        'login_page' => array(
          'Login page',
        ...
        'registered_page' => array(
          'Redirect registered surfers',
          'isNum',
          FALSE,
          'pageid',
          10,
          'Optional page to redirect surfers to if they are already registered.',
          0
        ),
    ...
    array(
      ...
      array(
        ...
        'message_format' => array(
          ...
          TRUE,
          'radio',
          array('text' => 'Text-only', 'html' => 'HTML and text'),
          '',
          'text'
        ),
        ...
        'message_html' => array(
          ...
          7,
          'Placeholders: NAME, TITLE, EMAIL, PROJECT, LINK',
          "Name: {%NAME%}<br />Title: {%TITLE%}<br />Email: {%EMAIL%}<br />
          Project: {%PROJECT%}<br />Link: {%LINK%}<br />"
        ),
  ...
  */
  function _initBaseSurfers() {
    if (!(isset($this->baseSurfers) && is_object($this->baseSurfers))) {
      $this->baseSurfers = new surfer_admin();
    }
  }
  ...
  */
  function getParsedData() {
    ...
    if ($surferObj->isValid) {
      if (isset($this->data['registered_page']) && (int)$this->data['registered_page'] > 0) {
        $href = $this->getWebLink($this->data['registered_page']);
  ...
  function getOutput() {
    ...
    if ($validationId != '') {
      ...
      if ($checkValidationTime == 0) {
        ...
        if (FALSE !== $surferId) {
          if (isset($this->data['auto_login']) && $this->data['auto_login'] == 1 &&
              isset($this->data['default_status']) && $this->data['default_status'] == 1) {
            $redirectPage = '';
            if ($this->data['auto_login_redirect'] > 0) {
              $redirectPage = $this->getWebLink($this->data['auto_login_redirect']);
            }
            $surferObj = $this->papaya()->surfer;
            $surferObj->executeLogin(FALSE, $surferId, $redirectPage);
          }
  ...
  */
  ...
  function makeValid($confirmString) {
    $this->_initBaseSurfers();
    return $this->baseSurfers->makeValid(
      $confirmString,
              ...
              if ($checkEmail === 0) {
                if (isset($this->data['use_summary']) && $this->data['use_summary'] == 1 &&
                  ...
                  if ($addUserCheck) {
                    ...
                    if ($this->data['upload_avatar'] == 1) {
                      ...
                      if (@isset($_FILES[$this->paramName]['tmp_name']['surfer_avatar'])) {
                        ...
                        if (isset($tempFileName) &&
                          ...
                          if ($tempFileSize <= 0) {
                            ...
                            if (!$fileId) {
                              ...
                              if ($mediaDB->databaseInsertRecord(
                                $mediaDB->tableFilesTrans,
                                NULL,
                                $data
                              ) == FALSE) {
                                $result .= $this->getErrorMessageXml(
                        ...
                        if (isset($mediaId) && !empty($mediaId) &&
                            PapayaFilterFactory::isGuid($mediaId, TRUE)) {
                    ...
                    $result .= $this->getErrorMessageXml($this->data['error_mail_send']);
                    $result .= $this->getOutputForm();
                  } elseif ($addUserCheck == -2) {
                    $result .= $this->getErrorMessageXml($this->data['error_database_input']);
                    $result .= $this->getOutputForm();
                  }
                }
              } else {
  ...
  */
  function addUser() {
    ...
    $surferData = array(
      ...
      'surfer_password' =>
        $this->baseSurfers->passwordApi()->getPasswordHash(
          $this->outputDialog->params['surfer_password_1']
        ),
    ...
    if ($insert && $confirm) {
      ...
      $actionData = array('surfer_id' => $newId);
      $actionsObj->call('community', 'onRegisterSurfer', $actionData);
  ...
  */
  function sendMail($confirmString, $emailDelayTime) {
    ...
    $repl['PROJECT'] = $this->papaya()->options->get('PAPAYA_PROJECT_TITLE', '');
    $page = NULL;
    ...
    if ($emailDelayTime == 0) {
      ...
      if ($this->data['message_format'] == 'html' && !empty($this->data['message_html'])) {
        $messageHtml = $this->data['message_html'];
      }
    } else {
      ...
      if ($this->data['delay_format'] == 'html' && !empty($this->data['delay_message_html'])) {
        $messageHtml = $this->data['delay_message_html'];
      }
    }
    $email = new email();
    $email->addAddress(
    ...
    $email->setReturnPath($this->data['mailfrom_email']);
    $email->setSubject($subject, $repl);
    $email->setBody($message, $repl);
    if ($messageHtml !== NULL) {
      $simpleTemplate = new base_simpletemplate();
      $messageHtml = $simpleTemplate->parse($messageHtml, $repl);
      $parser = new papaya_parser();
      $messageHtml = $parser->parse(
        $messageHtml,
        (int)$this->parentObj->topic['TRANSLATION']['lng_id']
      );
      $email->setBodyHTML($messageHtml);
    }
    $success = $email->send();
    if (!$success) {
      $this->logMsg(
  ...
  */
  function checkEmail() {
    ...
    if ($this->data['email_blacklist_check']) {
      if (
        $this->baseSurfers->checkEmailAgainstBlacklist(
          $this->outputDialog->params['surfer_email_1']
        )
      ) {
        return 0;
  ...
  */
  function getSummaryXML() {
    ...
    if (!empty($this->data['dynamic_class']) && is_array($this->data['dynamic_class'])) {
      ...
      if (!empty($dynFields)) {
        foreach ($dynFields as $fieldName => $dynField) {
          $result .= $this->getSummaryItem(
            'dyn_'.$fieldName, $fieldName, $dynField[0], $dynField[4]
          );
        }
      }
    }
    ...
    if (!empty($dynFields)) {
      foreach ($dynFields as $fieldName => $dynField) {
        $this->addSummaryFormItem($hidden, $fieldName);
      }
    }
    if ($this->data['agree_to_terms'] == 1) {
      $hidden['terms'] = 1;
  ...
  */
  function getSummaryItem($element, $field, $caption, $values = NULL) {
    ...
    if (isset($this->outputDialog->params[$field]) &&
      ...
      if (is_array($values) && isset($values[$this->outputDialog->params[$field]])) {
        $result = sprintf(
  ...
  */
  function callbackClasses($name, $element, $data) {
    ...
    $sql = "SELECT c.surferdataclass_id,
             ...
             ORDER BY c.surferdataclass_order";
    $sqlParams = array(
