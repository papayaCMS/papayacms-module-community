...
class surfer_admin extends base_db {
  ...
  */
   ...
   * @return surfer_admin $instance
   */
  public static function getInstance($paramName = "sadm") {
    static $instance;
    if (!(isset($instance) && is_object($instance))) {
      $instance = new surfer_admin($paramName);
    }
  ...
  function existID($id, $includeBlocked = FALSE) {
    if (PapayaFilterFactory::isGuid($id, TRUE)) {
      $sql = "SELECT COUNT(*)
               FROM %s
              WHERE surfer_id = '%s' %s";
  ...
  * @param boolean $includeBlocked Check blocked surfers too
  * @return array|string Surfer id(s) or empty
  */
  ...
  * @param boolean $includeBlocked Check blocked surfers too
  * @return array Surfer handle(s) or empty
  */
  function getHandleById($id, $order = FALSE, $sort = 'ASC', $includeBlocked = FALSE) {
    ...
    $loadIds = array();
    foreach ($id as $value) {
      if (PapayaFilterFactory::isGuid($value, TRUE)) {
  ...
  */
   ...
   * @param boolean $includeBlocked Check blocked surfers too
   * @return array|string Surfer name(s) or empty
   */
  ...
  * @see surfer_admin::existID() for behavior on blocked surfers
  * @access public
  * @param array|string $id Surfer id(s)
  * @param boolean $includeBlocked Check blocked surfers too
  * @return array|string Surfer name(s) or empty
  */
  ...
  * @see surfer_admin::existID() for behavior on blocked surfers
  * @access public
  * @param array|string $handles Surfer handle(s)
  * @param boolean $includeBlocked Check blocked surfers too
  ...
  * @return NULL|boolean|int|array $result Surfer(s)' status
  */
  ...
  */
  function getEmailDelayTime($email) {
    ...
    foreach ($emailBlacklist as $blacklistRule) {
  ...
  * @param int $mailExpiry Expiry of change request's confirmation email (in hours)
  * @return string|boolean Status (change request has been added)
  */
  function emailChangeRequest($surferId, $newEmail, $mailExpiry = 24) {
  ...
  * @return array|boolean Type => value results or FALSE
  */
  ...
  * @return NULL|array Surfer data or empty
  */
   ...
   * @param NULL|string $sort Sort results ('ASC' | 'DESC')
  ...
  * @see existID() for behavior on blocked surfers
  * @access public
  * @param int|NULL $limit Limit database results set
  * @param int|NULL $offset Offset of database results set
  * @param string|array $orderBy Order by fieldname or array (field => direction)
  ...
  */
   ...
   * @see existID() for behavior on blocked surfers
   * @access public
   * @param array|string $ids Unique 32-char surfer id
  ...
  * @param integer $language Default content language for the surfer (optional, default 0)
  * @return string surfer id on success, FALSE otherwise
  */
  function makeValid($confirmString, $newStatus = 1, $language = 0) {
    $surferId = '';
  ...
  */
  function createSurfer($data, $ignoreIllegal = FALSE) {
    ...
    if (isset($saveData['surfer_password'])) {
      $saveData['surfer_password'] =
        $this->passwordApi()->getPasswordHash($saveData['surfer_password']);
    }
  ...
  *
  * @access public
  * @param array $params Optional surfer data params, fallback $this->params
  ...
  function saveSurfer($params = NULL, $ignoreIllegal = FALSE) {
    ...
    if (isset($params) && $params !== NULL) {
      ...
      $data = array(
        ...
        'surfer_language' => isset($this->params['surfer_language']) ?
          (int)$this->params['surfer_language'] : NULL
      );
      // If the password was changed,
      // create its hash and add it to data
      if (isset($this->params['surfer_password']) &&
          trim($this->params['surfer_password']) != '') {
        $data['surfer_password'] =
          $this->passwordApi()->getPasswordHash($this->params['surfer_password']);
      }
    }
    ...
    if ($update !== FALSE) {
      ...
      $actionsObj->call('community', 'onModifySurfer', $actionData);
  ...
  */
  ...
  function searchSurfers($pattern, $handleOnly = FALSE,
    ...
    $searchMode = $this->papaya()->options->get('PAPAYA_SEARCH_BOOLEAN', 0);
    if ($filter = $parser->getSQL($pattern, $searchFields, $searchMode)) {
      $sql = "SELECT surfer_id,
                     surfer_handle,
                     surfer_email,
                     surfer_givenname,
  ...
  *
  * @access public
  * @param array $adminGroups optional, array of admin groups or NULL
  ...
  */
  function getLanguageTitle($lngId = NULL) {
    if ($lngId == NULL) {
      return $this->papaya()->administrationLanguage->code;
    } else {
      return $this->papaya()->languages->getLanguage($lngId)->code;
    }
  }
  ...
  * @return int|FALSE result of database operation or FALSE
  */
  ...
  */
  function addContactRequest($surferId, $contactId, $status = SURFERCONTACT_STATUS_PENDING) {
    ...
    if (
         !(
           PapayaFilterFactory::isGuid($surferId, TRUE) &&
           PapayaFilterFactory::isGuid($contactId, TRUE)
         )
       ) {
      return FALSE;
    }
  ...
  */
   ...
   *
   * @access public
   * @param array|string $surferIds
   * @return integer|array Surfer(s)' status
   */
  function getOnlineStatus($surferIds) {
    // Idle timeout (in seconds)
    $idleTimeout = (int)$this->getProperty('IDLE_TIMEOUT', 30) * 60;
    ...
    if ($res = $this->databaseQueryFmt($sql, $sqlParams)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($row['surfer_valid'] != 1) {
          // Surfer invalid?
          $stat = 0;
        } elseif (((int)$row['surfer_lastaction'] + $idleTimeout) < time()) {
          // Idle timeout?
          $stat = 0;
        }
  ...
  * @param array|string $surferIds Unique 32-char surfer id(s)
  * @param string $fieldName Dynamic data file name
  * @return array|string|NULL Dynamic data result
  */
  ...
  * @access public
  * @param array|string $surferIds Unique 32-char surfer id(s)
  * @param array|string|int $fields Dynamic data fields
  * @return array|string|NULL Dynamic data
  */
  ...
  *
  * @access public
  * @param array|string $surferIds Unique 32-char surfer id
  * @param array|string $fields Dynamic data field names
  * @return array|integer|NULL Data count(s) or empty
  */
  ...
  * @access public
  * @param string $surferId Unique 32-char surfer id
  * @param string|array $fields Dynamic data field name(s)
  ...
  * @param string $surferId Unique 32-char surfer id
  * @param string|array $fields Dynamic data field name(s)
  * @param string|NULL $value Dynamic data field value(s)
  ...
  * @access public
  * @param string|array $fields Dynamic data field name(s)
  * @param string|NULL $value Dynamic data field value
  * @return boolean|array Checked status result(s)
  */
  ...
  */
   ...
   *
   * @access public
   * @param string $xmlString XML input
  ...
  function parseFormValueXML($xmlString, $lng = 0, $complete = FALSE) {
    ...
    $xml = PapayaXmlDocument::createFromXML($xmlString);
    ...
    foreach ($xml->xpath()->evaluate('/*/value') as $valueNode) {
      $name = $xml->xpath()->evaluate('string(content)', $valueNode);
      if ($complete) {
        $captions = array();
        foreach ($xml->xpath()->evaluate('captions/*', $valueNode) as $captionNode) {
          $captions[$captionNode->localName] = $captionNode->nodeValue;
        }
        $values[$name] = $captions;
      } else {
        $values[$name] = $xml->xpath()->evaluate('string(captions/'.$lngName.')', $valueNode);
      }
    }
    return $values;
  }
  ...
  *
  * @access public
  * @param NULL|int|string|array $fields Field names
  ...
  */
  function getDynamicEditFields($fields, $prefix = '', $lng = 0) {
    ...
    $sql = "SELECT s.surferdata_id, s.surferdata_name, s.surferdata_type,
                   s.surferdata_values, s.surferdata_check, s.surferdata_class,
                   s.surferdata_available, s.surferdata_mandatory, s.surferdata_order,
                   c.surferdataclass_order
              FROM %s s
             INNER JOIN %s c
                ON s.surferdata_class = c.surferdataclass_id
             WHERE s.surferdata_available = 1".
                   $fieldCondition."
          ORDER BY c.surferdataclass_order, s.surferdata_order, s.surferdata_name";
  ...
  */
   ...
   *
   * @access public
   * @param PapayaTemplate $layout Papaya layout object
   * @param string $paramName Parameter group name
   * @param NULL|array $hidden Hidden formular values
  ...
  *
  * @access public
  * @return string|NULL Default surfer contact value
  */
   ...
   * @return bool
   */
  public function addSurferToFavorites($surferId) {
    ...
    return FALSE !== $this->databaseInsertRecord($this->tableFavorites, NULL, $data);
  }
  ...
  */
   ...
   * @param integer $limit Set limit for database results
   * @param integer $offset Set offset for database results
   * @return array $results List with latest registered surfers data
   */
  ...
  */
  ...
  function getBackendSurferLink($surferId) {
    ...
    $link = sprintf(
      $this->papaya()->options->get('PAPAYA_PATHWEB_ADMIN').
        'module_edmodule_community.php?%1$s[mode]=0&%1$s[id]=%2$s',
      $this->paramName,
      $surferId
    );
    return $link;
  ...
  */
  ...
  /**
  * Check whether a surfer already has an API key
  *
  * @param string $surferId
  * @return boolean TRUE if the surfer has an API key, otherwise FALSE
  */
  function hasApiKey($surferId) {
    $result = FALSE;
    $sql = "SELECT surfer_id, surfer_apikey
              FROM %s
             WHERE surfer_id = '%s'";
    $sqlParams = array($this->tableSurfer, $surferId);
    if ($res = $this->databaseQueryFmt($sql, $sqlParams)) {
      if ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        if (!empty($row['surfer_apikey'])) {
          $result = TRUE;
        }
      }
    }
  ...
  * @return string new API key or boolean FALSE if API key exists and $ovveride === FALSE
  */
  function createApiKey($surferId, $override = FALSE) {
    if ($override === FALSE) {
      if ($this->hasApiKey($surferId)) {
        return FALSE;
      }
    }
    $apiKey = md5($surferId.PapayaUtilRandom::getId());
    $success = $this->databaseUpdateRecord(
      $this->tableSurfer,
      array('surfer_apikey' => $this->passwordApi()->getPasswordHash($apiKey)),
      'surfer_id',
      $surferId
    );
    if (FALSE !== $success) {
