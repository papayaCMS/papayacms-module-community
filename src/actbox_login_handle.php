<?php
/**
* Actionbox - Login by handle form
*
...
* @version $Id: actbox_login_handle.php 39732 2014-04-08 15:34:45Z weinert $
*/
...
* @subpackage _Base-Community
*/
class actionbox_login_handle extends base_actionbox {
  ...
  */
  var $editFields = array(
    ...
    'login_page' => array(
      ...
      'pageid',
      10,
      'Optional link to this page to retrieve forgotten passwords',
      0
    ),
    'register_page' => array(
      'Registration page',
      ...
      'pageid',
      10,
      'Optional link to this page for unregistered users',
      0
    ),
    'profile_page' => array(
      'Profile page',
  ...
  function getParsedData() {
    ...
    if ($surferObj->isValid) {
      ...
      $result .= $surferObj->getHandleFormXML(
        ...
        empty($this->data['caption_handle'])
          ? '' : papaya_strings::escapeHTMLChars($this->data['caption_handle']),
        empty($this->data['caption_password'])
    ...
    if ($surferObj->isValid) {
      ...
      $result .= sprintf(
        ...
        empty($this->data['caption_logout_button'])
      ...
      $result .= sprintf(
        '<login_description>%s</login_description>'.LF,
        $this->getXHTMLString($this->data['description_login'], TRUE)
      );
      $result .= sprintf(
        '<login_button>%s</login_button>'.LF,
        empty($this->data['caption_login_button'])
    ...
    if (!($surferObj->isValid)) {
      $check = (
        isset($_POST[$surferObj->logformVar.'_handle']) &&
        isset($_POST[$surferObj->logformVar.'_password'])
      ) || (
        isset($_POST[$surferObj->logformVar]['handle']) &&
