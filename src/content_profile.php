...
* @version $Id: content_profile.php 39732 2014-04-08 15:34:45Z weinert $
*/
...
class content_profile extends base_content {
  ...
  */
  function saveProfileData() {
    ...
    if ($this->baseSurfers->saveSurfer($this->surferData)) {
      if (isset($this->data['dynamic_class']) && is_array($this->data['dynamic_class']) &&
          !empty($this->data['dynamic_class'])) {
        // Get the field names
        $dynFieldNames = $this->baseSurfers->getDataFieldNames($this->data['dynamic_class']);
        // Get data from these fields
        $dynData = $this->baseSurfers->getDynamicData($this->surferObj->surferId, $dynFieldNames);
        $dynFields = array();
        ...
        if (!empty($dynFields)) {
          ...
          $dynData = array_merge($dynData, $dynFields);
          return $this->baseSurfers->setDynamicData($this->surferObj->surferId, $dynData);
        }
      }
      return TRUE;
    }
  ...
  function getParsedData($parseParams = NULL) {
    ...
    if (isset($this->surferObj) &&
            ...
            if (isset($this->data['dynamic_class']) && is_array($this->data['dynamic_class']) &&
              ...
              foreach ($checkDynamic as $fieldName => $fieldResult) {
                if ($fieldResult == FALSE) {
                  $this->profileForm->inputErrors['dynamic_'.$fieldName] = 1;
                }
              }
            }
      ...
      if (!(
        ...
        $timestamps = '<timestamps>'.LF;
        $timestamps .= sprintf(
          '<timestamp type="registration" value="%d">%s</timestamp>'.LF,
          $this->surferObj->surfer['surfer_registration'],
          date('Y-m-d H:i:s', $this->surferObj->surfer['surfer_registration'])
        );
        $timestamps .= sprintf(
          '<timestamp type="last-login" value="%d">%s</timestamp>'.LF,
          $this->surferObj->surfer['surfer_lastlogin'],
          date('Y-m-d H:i:s', $this->surferObj->surfer['surfer_lastlogin'])
        );
        $timestamps .= sprintf(
          '<timestamp type="last-action" value="%d">%s</timestamp>'.LF,
          $this->surferObj->surfer['surfer_lastaction'],
          date('Y-m-d H:i:s', $this->surferObj->surfer['surfer_lastaction'])
        );
        $timestamps .= '</timestamps>'.LF;
        $result .= sprintf(
          '<title>%s</title>'.LF.
