...
* @version $Id: content_login.php 39813 2014-05-12 15:12:21Z weinert $
*/
...
class content_login extends base_content {
  ...
  var $editGroups = array(
    ...
    array(
      ...
      array(
        ...
        'Caption_Register_Headline' => array(
          'Register headline',
          'isNoHTML',
          TRUE,
          'input',
  ...
  *
  * @access private
  * @return surfer_admin instance of class surfer_admin
  */
  function _initSurferAdmin() {
    if (!(isset($this->surferAdmin) && is_object($this->surferAdmin))) {
      $this->surferAdmin = new surfer_admin();
    }
  ...
  */
  function getParsedData() {
    ...
    if (!empty($this->data['Caption_Register_Headline'])) {
      $result .= sprintf(
        '<register-headline>%s</register-headline>'.LF,
        papaya_strings::escapeHTMLChars($this->data['Caption_Register_Headline'])
      );
    }
    $result .= '</captions>'.LF;
    ...
    if ($mailChange && $mailChangeSuccess) {
      ...
      if (isset($this->params['newpwd'])) {
        ...
        if ($this->surferObj->requestPasswordChange($this->data, $resetBy, $passwordPage)) {
          ...
          $result .= sprintf(
            '<text>%s</text>',
            $this->getXHTMLString($this->data['text_change_requested'])
          );
          return $result;
      ...
      if ($token != NULL) {
        if ($surferData = $this->surferObj->checkChangePasswordId($token)) {
          ...
          if ($passwordChange) {
            $surferAdmin = $this->_initSurferAdmin();
            $policyCheck = $surferAdmin->checkPasswordForPolicy($password);
            if ($policyCheck === 0
              ...
              if ($policyCheck + 4 <= 0) {
                $message = $this->data['Error_Password_Blacklisted'];
              } elseif ($policyCheck + 1 <= 0) {
                $message = $this->data['Error_Password_Short'];
              } else {
                $message = $this->data['Error_Password_Match'];
              }
              $result .= sprintf(
                '<error>%s</error>',
                papaya_strings::escapeHTMLChars($message)
  ...
  */
  function changePassword($surferData, $password) {
    $surferAdmin = $this->_initSurferAdmin();
    $data = array(
      'surfer_password' => $this->surferObj->passwordApi()->getPasswordHash($password)
    );
    ...
    if (isset($changeType) && $changeType == 'email') {
      ...
      if ($surferEmailNew != '' && time() <= (int)$expiryTime) {
        ...
        if ($this->surferObj->isValid) {
          ...
          $this->surferObj->loadLogin($surferEmailNew);
  ...
  */
  function relocate() {
    $href = $this->getAbsoluteURL((int)$this->data['topic']);
