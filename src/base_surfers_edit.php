...
class surfer_admin_edit extends surfer_admin {
  ...
  */
  ...
  /**
  * Dialog to create a surfer API key
  * @var base_dialog
  */
  var $apiKeyDialog = NULL;
   ...
   * @var bool
   */
  public $allowNoSurfer = FALSE;

  /**
  * Constructor
  */
  function __construct($paramName = "sadm") {
    ...
    if ($this->params['mode'] == 1) {
      ...
      if ($this->module->hasPerm(4, TRUE)) {
        ...
        case 'export_fields' :
          ...
          $this->getExportProfileForm($layout);
          break;
        case 'import_fields' :
          // Display dialog to import dynamic data fields
          $this->getImportProfileForm($layout);
          break;
        case 'add_value' :
  ...
  */
  function insertSurfer() {
    $newSurferId = $this->createSurferId();
    $now = time();
    $data = array(
      'surfer_id' => $newSurferId,
      ...
      'surfer_password' => $this->passwordApi()->getPasswordHash(@$this->params['surfer_password']),
    ...
    if (FALSE !== $this->databaseInsertRecord($this->tableSurfer, NULL, $data)) {
      return $newSurferId;
    }
    return FALSE;
  }
    ...
    if (!$new) {
      ...
      if ($hasTitles) {
        ...
        foreach ($irows as $irow) {
          ...
          $titleList .= sprintf(
            '<subitem align="center"><a href="%s"><glyph src="%s" hint="%s"/></a></subitem>'.LF,
            papaya_strings::escapeHTMLChars($deleteHref),
  ...
  */
  function getEditTitleForm($layout, $new = FALSE) {
    ...
    if (!$new) {
      ...
      $data['language'] = $this->papaya()->administrationLanguage->id;
    }
    // Create dialog
  ...
  function getEditFieldPropertiesForm($layout) {
    ...
    if ($fieldType == 'input') {
      ...
      $listView .= sprintf(
        ...
        papaya_strings::escapeHTMLChars($images['actions-table-row-add']),
  ...
  */
  function getSelectProfileDropdown() {
    ...
    $selectFieldForm->dialogTitle = $this->_gt('Add. list field');
  ...
  */
  function getProfileCategoryList() {
    ...
    $lng = $this->papaya()->administrationLanguage->id;
    ...
    $classes = array();
    if ($res = $this->databaseQueryFmt($sql, $this->tableDataClasses)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        $classes[] = array(
          'id' => $row['surferdataclass_id'],
    ...
    if (count($classes) > 0) {
      ...
      $result .= '</cols>'.LF;
      $result .= '<items>'.LF;
      // Get last element's index to emit last down button
      ...
      for ($i = 0; $i <= $last; $i++) {
        ...
        $selected = '';
        if (isset($this->params['class_id']) && $this->params['class_id'] == $class['id']) {
        ...
        if ($i > 0) {
          $result .= sprintf(
            '<subitem align="center"><a href="%s"><glyph src="%s" hint="%s"/></a></subitem>'.LF,
            papaya_strings::escapeHTMLChars($upHref),
            papaya_strings::escapeHTMLChars($images['actions-go-up']),
        ...
        if ($i < $last) {
          $result .= sprintf(
            '<subitem align="center"><a href="%s"><glyph src="%s" hint="%s"/></a></subitem>'.LF,
            papaya_strings::escapeHTMLChars($downHref),
            papaya_strings::escapeHTMLChars($images['actions-go-down']),
  ...
  function getProfileList() {
    ...
    $sql = "SELECT s.surferdata_id, s.surferdata_name,
             ...
             ORDER BY s.surferdata_class, s.surferdata_order,
                      s.surferdata_name";
    $rows = array();
    if ($res = $this->databaseQueryFmt($sql, array($this->tableData))) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        $rows[] = $row;
      }
    }
    ...
    if (count($rows) > 0) {
      ...
      for ($i = 0; $i <= $last; $i++) {
        ...
        $selected = '';
        if (isset($this->params['data_id']) &&
            $this->params['data_id'] == $row['surferdata_id']) {
          $selected = ' selected="selected"';
        }
        $result .= sprintf(
          '<listitem href="%s" title="%s" indent="1" image="%s"%s>'.LF,
          papaya_strings::escapeHTMLChars($editHref),
          papaya_strings::escapeHTMLChars($row['surferdata_name']),
        ...
        $fieldTitle = sprintf('[%s]', $this->_gt('untitled'));
        $sql = "SELECT st.surferdatatitle_title
                  FROM %s AS st
                 WHERE st.surferdatatitle_field=%d
        ...
        if (
          ...
          if ($irow = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
            $fieldTitle = $irow['surferdatatitle_title'];
          }
        }
        $result .= $fieldTitle;
        $result .= '</subitem>'.LF;
        ...
        // Check whether we need to emit the down arrow
        if ($i == $last ||
  ...
  */
  function initializeSurferForm($surfer, $mode = 'edit', $force = FALSE) {
    ...
    if (!(isset($this->surferDialog) && is_object($this->surferDialog))) {
      ...
      if ($basicDataMode) {
        ...
        if (isset($surfer) && $mode == 'edit') {
          ...
          $data = array(
            ...
            'surfer_status' => isset($surfer['surfer_status']) && $surfer['surfer_status'] != 0 ?
              'online' : 'offline',
            'surfer_language' =>
              isset($surfer['surfer_language']) && $surfer['surfer_language'] != 0 ?
              $surfer['surfer_language'] : 0
          );
        } else {
        ...
        $maxLength = $this->papaya()->options->get('PAPAYA_COMMUNITY_HANDLE_MAX_LENGTH', 20);
        $fields = array(
          'surfer_email' =>
            array('Email', 'isEMail', TRUE, 'input', 200, ''),
          'surfer_handle' =>
            array('Username', '/^[a-z\d._-]{4,'.$maxLength.'}$/i',
                  TRUE,
                  'input', 200,
          ...
          'surfer_language' =>
            array('Preferred language', 'isNum', FALSE, 'function', 'callbackLanguages', '', 0),
          'Options',
        ...
        if ($changereq != NULL) {
          ...
          foreach ($changereq as $req) {
            ...
            $output .= sprintf(
              ...
              papaya_strings::escapeHTMLChars($images[$icon]),
  ...
  /**
  * Create a select field to choose a group
  *
  * This is a callback function called by base_dialog in order to
  * load dynamic field values for the following field types:
  *
  * <ul>
  *   <li>combo boxes</li>
  *   <li>radiobuttons</li>
  ...
  */
  ...
  /**
  * Callback method to select a content language
  *
  * @param string $name
  * @param array $field
  * @param integer $value
  * @return string form XML
  */
  public function callbackLanguages($name, $field, $value) {
    $languageSelect = new base_language_select();
    return $languageSelect->getContentLanguageCombo(
      $this->paramName,
      $name,
      $field,
      $value,
      sprintf('[%s]', $this->_gt('Please select'))
    );
  }
  ...
  */
  function editSurfer($surfer) {
    ...
    $result = '';
    $surfMode = isset($this->params['surfmode']) ? $this->params['surfmode'] : 0;
    if ($surfMode == 0 || $surfMode == 2) {
      $result = $this->surferDialog->getDialogXML();
    }
    return $result;
  }
    ...
    if (!empty($rules)) {
      ...
      foreach ($rules as $id => $rule) {
        ...
        if ($rule['blacklist_delay'] == 1) {
          $cmd = 'delay_off';
          $icon = $images['status-node-checked'];
          $hint = $this->_gt('Block email addresses matching to this rule');
        } else {
          $cmd = 'delay_on';
          $icon = $images['status-node-empty'];
      ...
      foreach ($this->permissionsList as $permId => $perm) {
        ...
        case 'show':
          ...
          if ($perm['surferperm_active']) {
            $result .= sprintf(
              '<glyph src="%s" />',
              papaya_strings::escapeHTMLChars($images['status-node-checked-disabled'])
            );
          } else {
            $result .= sprintf(
              '<glyph src="%s" />',
              papaya_strings::escapeHTMLChars($images['status-node-empty-disabled'])
            );
          }
          $result .= '</subitem>';
          $result .= '<subitem align="center">';
  ...
  */
   ...
   *
   * @param array $list
   * @param integer $numIncs optional, default value NULL
    ...
    if (isset($this->surfers) && is_array($this->surfers)) {
      ...
      foreach ($shortSurf as $surfer) {
        ...
        $result .= sprintf(
          ...
          papaya_strings::escapeHTMLChars($surfer['surfer_handle']),
          papaya_strings::escapeHTMLChars(
            $images[$this->statusImages[$surfer['surfer_valid']]]
          ),
          $selected
        );
        ...
        if ($surfer['surfer_email'] != '') {
          $result .= sprintf(
            ...
            papaya_strings::escapeHTMLChars($images['items-mail']),
            papaya_strings::escapeHTMLChars(
    ...
    if ($max > $step) {
      ...
      if ($currentPage >= 5) {
        ...
        $result .= sprintf(
          ...
          papaya_strings::escapeHTMLChars($images['actions-go-previous-fast']),
          papaya_strings::escapeHTMLChars(
      ...
      if ($currentPage > 0) {
        ...
        $result .= sprintf(
          ...
          papaya_strings::escapeHTMLChars($images['actions-go-previous']),
          papaya_strings::escapeHTMLChars(
            $this->getLink($this->addFilterParams(array('cmd' => 'show', $paramName => $i)))
          )
        );
      ...
      if ($currentPage < $pageCount - 1) {
        ...
        $result .= sprintf(
          ...
          papaya_strings::escapeHTMLChars($images['actions-go-next']),
          papaya_strings::escapeHTMLChars(
            $this->getLink($this->addFilterParams(array('cmd' => 'show', $paramName => $i)))
          )
        );
      ...
      if ($currentPage < $pageCount - 5) {
        ...
        $result .= sprintf(
          ...
          papaya_strings::escapeHTMLChars($images['actions-go-next-fast']),
          papaya_strings::escapeHTMLChars(
            $this->getLink($this->addFilterParams(array('cmd' => 'show', $paramName => $i)))
          )
        );
    ...
    if ($this->params['mode'] == 1) {
      ...
      if ($this->module->hasPerm(6)) {
        if (isset($this->editSurfer['surfer_id'])
          ...
          $toolbar->addSeperator();
          if ($this->papaya()->administrationUser->hasPerm(4)
              && trim($this->editSurfer['auth_user_id']) == '') {
            $toolbar->addButton(
              'Add editor user',
              $this->getLink(
          ...
          if ($this->papaya()->administrationUser->hasPerm(6)) {
            $toolbar->addButton(
              'Generate API Key',
              $this->getLink(
                array(
              ...
              'actions-link-add',
              'Assign an API key for this surfer',
              (isset($this->params['cmd']) && $this->params['cmd'] == 'add_apikey')
            );
          }
        }
      }
  ...
  */
  function importProfileFields() {
    ...
    $xml = PapayaXmlDocument::createFromXml(file_get_contents($tempFileName));
    ...
    $classRecords = array();
    /** @var PapayaXmlElement $class */
    foreach ($xml->xpath()->evaluate('/profile-data/data-classes/data-class[@id]') as $class) {
    ...
    $classTitleNodes = $xml->xpath()->evaluate(
      '/profile-data/data-class-titles/data-class-title[@class-id]'
    );
    /** @var PapayaXmlElement $classTitle */
    foreach ($classTitleNodes as $classTitle) {
      $classId = @(int)$classTitle->getAttribute('class-id') + $classOffset;
      $lang = @(int)$classTitle->getAttribute('lang');
      $title = $classTitle->nodeValue;
    ...
    if (count($classTitleRecords) > 0) {
      ...
      $this->addMsg(
        ...
        sprintf(
          ...
          count($classTitleRecords)
        )
      );
    }
    // For add mode, get all existing field names to leave them out
    ...
    /** @var PapayaXmlElement $dataField */
    foreach ($xml->xpath()->evaluate('/profile-data/data-fields/data-field[@id]') as $dataField) {
      ...
      $name = $xml->xpath()->evaluate('string(name)', $dataField);
      $values = $xml->xpath()->evaluate('string(values)', $dataField);
    ...
    if (count($dataFieldRecords) > 0) {
      ...
      $this->addMsg(
        ...
        sprintf(
          ...
          count($dataFieldRecords)
        )
      );
    }

    $fieldTitleRecords = array();
    $fieldTitleNodes = $xml->xpath()->evaluate(
      '/profile-data/data-field-titles/data-field-title[@field]'
    );
    /** @var PapayaXmlElement $fieldTitle */
    foreach ($fieldTitleNodes as $fieldTitle) {
      $fieldId = @(int)$fieldTitle->getAttribute('field') + $fieldOffset;
      $lang = @(int)$fieldTitle->getAttribute('lang');
      $title = $fieldTitle->nodeValue;
   ...
   * @param int $newField id of a new field that doesn't have an order number yet
   */
   ...
   *
   * @param int $newClass id of a new category that doesn't have an order number yet
   ...
   *
   * @access public
   * @param bool|string $assoc optional
   * @param bool $shortOnly
   * @return array
   */
  ...
  function callbackFolders($name, $field, $data) {
    ...
    $result = '';
    $folders = $this->mediaDB->getFolderComboArray(
      $this->papaya()->administrationLanguage->id
    );
    $result .= sprintf(
      '<select name="%s[%s]" class="dialogSelect dialogScale">'.LF,
  ...
  * provided right in this class
  ...
  function cleanupContacts() {
    ...
    if ($res = $this->databaseQueryFmt($sql, $params)) {
      while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
        if (PapayaFilterFactory::isGuid($row['surfercontact_requestor'], TRUE) &&
            $row['surfercontact_requestor'] != $row['surfer1']) {
          $removedSurfers[] = $row['surfercontact_requestor'];
        }
        if (PapayaFilterFactory::isGuid($row['surfercontact_requested'], TRUE) &&
  ...
  */
  function convertBlacklistEntries() {
    ...
    $this->addMsg(MSG_INFO, $this->_gtf('%d blacklist entries updated.', array($i)));
  }
  ...
  */
  ...
  /**
  * Get dialog to add an API key for the current surfer
  *
  */
  function getApiKeyDialog() {
    if (isset($this->params['confirm_apikey']) && $this->params['confirm_apikey'] == 1) {
      return;
    }
    $this->initializeApiKeyDialog();
    if (is_object($this->apiKeyDialog)) {
      $this->layout->add($this->apiKeyDialog->getDialogXML());
    }
  }
  ...
  */
  function initializeApiKeyDialog() {
    if (is_object($this->apiKeyDialog)) {
      return;
    }
    $fields = array(
    ...
    $hidden = array(
      ...
      'confirm_apikey' => 1,
      'cmd' => 'add_apikey',
      'patt' => isset($this->params['patt']) ? $this->params['patt'] : '',
      'offset' => isset($this->params['offset']) ? (int)$this->params['offset'] : '',
      'id' => $this->editSurfer['surfer_id']
    );
    $data = array();
    $this->apiKeyDialog = new base_dialog($this, $this->paramName, $fields, $data, $hidden);
    if (is_object($this->apiKeyDialog)) {
      $this->apiKeyDialog->dialogTitle = sprintf(
        $this->_gt("Create API key for '%s'"),
        $this->editSurfer['surfer_handle']
      );
      $this->apiKeyDialog->buttonTitle = $this->_gt('Create');
      $this->apiKeyDialog->loadParams();
    }
  }

  /**
  * Add API key for current surfer
  */
  public function addApiKey() {
    ...
    $this->initializeApiKeyDialog();
    $override = isset($this->apiKeyDialog->params['override']) &&
      $this->apiKeyDialog->params['override'] == 1;
    $apiKey = $this->createApiKey($this->editSurfer['surfer_id'], $override);
    if ($apiKey === FALSE) {
      $this->addMsg(
        MSG_WARNING,
        $this->_gt('Surfer already has an API key; check override if necessary.')
      );
    } else {
      $this->addMsg(
        MSG_INFO,
        sprintf($this->_gt('New API key: %s'), $apiKey)
      );
    }
  }
}